...
class Solution{
  private:
    void append(Node** head_ref, int c, int p)
    {
        if(*head_ref == NULL)
        {
            *head_ref = new Node(c,p);
            return;
        }
        Node* current = *head_ref;
        while(current->next!=NULL)
            current = current->next;
        current->next = new Node(c,p);
    }
  public:
    ...
    Node* addPolynomial(Node *p1, Node *p2)
    {
        Node* l = p1, *r = p2, *add=NULL;
        while(l!=NULL && r!=NULL)
        {
            if(l->pow > r->pow)
            {
                append(&add,l->coeff, l->pow);
                l = l->next;
            }
            else if(l->pow < r->pow)
            {
                append(&add, r->coeff, r->pow);
                r = r->next;
            }
            else
            {
                append(&add, l->coeff + r->coeff, l->pow);
                l = l->next;
                r = r->next;
            }
        }
